...
if __name__ == "__main__":
    ...
    for epoch in range(1, num_epochs + 1): # epoch numbers start at 1
        ...
        if config.experiment.rollout.enabled and epoch % rollout_every_n_epochs == 0:
            ...
            # print("all_rollout_logs: ", all_rollout_logs.keys())
            # OrderedDict([('Lift', {'Return': 0.0, 'Horizon': 300.0, 'Success_Rate': 0.0, 'uncertainty': -18.98784414768219, 'log_prob': -19.916816205978392, 'time': 3.1941628456115723, 'Time_Episode': 0.10647209485371907})])
            key = list(all_rollout_logs.keys())[0]
            success_rate = all_rollout_logs[key]["Success_Rate"]
        ...
        if config.experiment.save.enabled and epoch % config.experiment.save.every_n_epochs == 0:
            #TODO:implement save on best validation loss
    ...
    train_save_path = f"{log_dir}/train_loss_{task_name}_{num_epochs}_epochs_warmup_{warmup_epochs}_radial_flow.png"
    rollout_save_path = f"{log_dir}/rollout_success_{task_name}_{num_epochs}_epochs_warmup_{warmup_epochs}_radial_flow.png"
    plot_losses(train_losses, epochs, val_losses, val_epochs, train_save_path)

    # plot the rollout successes
    plt.figure(figsize=(10, 6))
    plt.plot(rollout_epochs, rollout_successes, marker='o', linestyle='-', linewidth=1.5, label='Rollout Success Rate')
    plt.xlabel("Epoch")
    ...
    print(f"Loss plot saved to {rollout_save_path}")
